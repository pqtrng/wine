...
def plot_residual(x_test, y_test, model, image_config):
    ...
    res_df = pd.DataFrame(list(zip(y_jitter, y_pred)),
                          columns=["true", "pred"])
    ...
    ax.set_ylabel('Predicted wine quality',
                  fontsize=image_config.axis_fs)  # ylabel
    ax.set_title('Residuals', fontsize=image_config.title_fs)

    # Make it pretty- square aspect ratio
    ax.plot(image_config.x_size, image_config.y_size,
            image_config.color, linewidth=image_config.line_width)
...
def plot_feature(model, labels, image_config):
    ...
    ax.set_title('Random forest\nfeature importance',
                 fontsize=image_config.title_fs)
